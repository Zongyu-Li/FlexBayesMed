#' Cumulative Distribution Function (CDF) of the Centred Two-Piece Student's t-Distribution (CTPT)
#'
#' Computes the CDF for the Mean-Adjusted Two-Piece Student's t-Distribution (CTPT),
#' a skewed heavy-tailed distribution with zero mean used in robust mediation analysis.
#'
...
pctpt <- function(q, gamma, nu, lower.tail = TRUE, log.p = FALSE) {
  ...
  if (is.character(nu)) {
    ...
    stop("For Student's t sampling, nu must be > 1.")
  }
