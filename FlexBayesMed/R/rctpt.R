#' Generate Samples from the Centred Two-Piece Student \emph{t}-Distribution (CTPT)
#'
#' Produces independent and identically distributed (i.i.d.) samples from the CTPT distribution,
#' which is a zero-mean adjusted version of the two-piece Student \eqn{t}-distribution (or normal).
...
#'
#' @examples
#' # Generate samples from CTPT with gamma=2, nu=5 (Student t case)
#' samples_t <- rctpt(N = 1000, gamma = 2, nu = 5)
#' hist(samples_t, breaks = 30, main = "CTPT (nu=5)")
#'
#' # Generate samples from CTPT with gamma=2 (normal case)
#' samples_norm <- rctpt(N = 1000, gamma = 2, nu = "normal")
#' hist(samples_norm, breaks = 30, main = "CTPT (normal)")
#'
#' @seealso
